...
describe('Gym Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./gym-detail.component').then(m => m.GymDetailComponent),
              resolve: { gym: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load gym on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', GymDetailComponent);

      // THEN
      expect(instance.gym()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
